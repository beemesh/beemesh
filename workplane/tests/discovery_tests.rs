...
fn put_and_list_returns_record() {
    ...
    let records = discovery::list("default", &workload);
...
fn list_returns_empty_for_unknown_workload() {
    let records = discovery::list("nonexistent", "unknown-workload");
...
fn namespace_isolation() {
    ...
    let ns1_records = discovery::list("namespace-1", &workload);
    let ns2_records = discovery::list("namespace-2", &workload);

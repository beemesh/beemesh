...
async fn test_keystore_shared_across_threads() {
    ...
    let handle1 = tokio::spawn(async move {
        ...
        result1
            .lock()
            .unwrap()
            .push(format!("Thread1 stored and found: {:?}", cids));
    });
    ...
    let handle2 = tokio::spawn(async move {
        ...
        result2
            .lock()
            .unwrap()
            .push(format!("Thread2 found: {:?}", cids));
        ...
        if cids.contains(&"test_cid_123".to_string()) {
            ...
            result2
                .lock()
                .unwrap()
                .push(format!("Thread2 retrieved blob: {:?}", blob));
        }
    });
    ...
    let found_by_thread2 = results
        .iter()
        .any(|r| r.contains("Thread2 found:") && r.contains("test_cid_123"));
    assert!(
        found_by_thread2,
        "Thread 2 should be able to see data stored by Thread 1 in shared keystore"
    );
}

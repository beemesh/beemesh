...
use machineplane::messages::signatures::{
    sign_award, sign_bid, sign_scheduler_event, sign_tender, verify_sign_award, verify_sign_bid,
    verify_sign_scheduler_event, verify_sign_tender,
};
use machineplane::messages::types::{Award, Bid, EventType, SchedulerEvent, Tender};
...
/// Creates a valid signed award for testing.
fn create_signed_award(
    keypair: &Keypair,
    tender_id: &str,
    winners: Vec<String>,
    timestamp: u64,
    ...
    let mut award = Award {
        tender_id: tender_id.to_string(),
        winners,
        manifest_digest: format!("digest-{}", tender_id),
        timestamp,
        nonce,
        signature: Vec::new(),
    };
    sign_award(&mut award, keypair).expect("award signing should succeed");
    award
}
...
/// Verifies that signed awards are accepted.
#[test]
fn signature_verification_accepts_valid_award() {
    let keypair = Keypair::generate_ed25519();
    let public_key = keypair.public();

    let award = create_signed_award(
        &keypair,
        "tender-1",
    ...
    assert!(
        verify_sign_award(&award, &public_key),
        "Valid signed award MUST be accepted"
    );
}

/// Verifies that tampered awards are rejected.
#[test]
fn signature_verification_rejects_tampered_award() {
    let keypair = Keypair::generate_ed25519();
    let public_key = keypair.public();

    let mut award = create_signed_award(
        &keypair,
        "tender-1",
    ...
    // Tamper with the winners list
    award.winners.push("injected-node".to_string());

    assert!(
        !verify_sign_award(&award, &public_key),
        "Tampered award MUST be rejected"
    );
}
...
fn selection_window_is_reasonable() {
    use machineplane::messages::constants::DEFAULT_SELECTION_WINDOW_MS;
...
/// Verifies that changing any award field invalidates the signature.
#[test]
fn award_signature_covers_all_fields() {
    let keypair = Keypair::generate_ed25519();
    let public_key = keypair.public();

    let original = create_signed_award(
        &keypair,
        "tender-1",
        vec!["node-a".to_string()],
        now_ms(),
    ...
    tampered_tender_id.tender_id = "different-tender".to_string();
    assert!(
        !verify_sign_award(&tampered_tender_id, &public_key),
        "Changing tender_id MUST invalidate signature"
    );

    let mut tampered_winners = original.clone();
    tampered_winners.winners = vec!["different-winner".to_string()];
    assert!(
        !verify_sign_award(&tampered_winners, &public_key),
        "Changing winners MUST invalidate signature"
    );

    let mut tampered_digest = original.clone();
    tampered_digest.manifest_digest = "different-digest".to_string();
    assert!(
        !verify_sign_award(&tampered_digest, &public_key),
        "Changing manifest_digest MUST invalidate signature"
    );
}
...
fn fabric_topic_constant_is_correct() {
    use machineplane::messages::constants::BEEMESH_FABRIC;
...
// ============================================================================
// Award Winner List Tests
// ============================================================================

/// Verifies that award contains non-empty winner list.
#[test]
fn award_has_winners() {
    let keypair = Keypair::generate_ed25519();
    let winners = vec!["node-a".to_string(), "node-b".to_string()];
    let award = create_signed_award(&keypair, "tender-1", winners.clone(), now_ms(), 12345);

    assert_eq!(award.winners.len(), 2, "Award MUST contain winner list");
    assert!(award.winners.contains(&"node-a".to_string()));
    assert!(award.winners.contains(&"node-b".to_string()));
}

/// Verifies that award with empty winners is valid (no qualifying bids).
#[test]
fn award_can_have_no_winners() {
    let keypair = Keypair::generate_ed25519();
    let public_key = keypair.public();

    let award = create_signed_award(&keypair, "tender-1", vec![], now_ms(), 12345);

    assert!(award.winners.is_empty(), "Award MAY have no winners");
    assert!(
        verify_sign_award(&award, &public_key),
        "Award with no winners MUST still be valid"
    );
}

...
async fn decrypt_encrypted_manifest_local_only(
    encrypted_manifest_bytes: &[u8],
) -> Result<String, anyhow::Error> {
    use base64::Engine as _;

    // Parse the FlatBuffer to extract nonce and payload
    let encrypted_manifest =
        protocol::machine::root_as_encrypted_manifest(encrypted_manifest_bytes)?;
    ...
    // Calculate manifest ID from the FlatBuffer content
    let mut hasher = DefaultHasher::new();
    encrypted_manifest_bytes.hash(&mut hasher);
    let manifest_id = format!("{:x}", hasher.finish());

    log::info!(
        "libp2p: decrypt attempting to recover key shares for manifest_id={}",
        manifest_id
    );
    ...
    log::info!(
        "libp2p: decrypt found {} local key shares for manifest_id={}",
        key_shares.len(),
        manifest_id
    );
    ...
    if key_shares.len() >= 2 {
        ...
        return Err(error_helpers::not_enough_local_key_shares(
            2 - key_shares.len(),
        ));
    }
    ...
    log::info!(
        "libp2p: decrypt successfully decrypted manifest to YAML (len={})",

    ...
    F: FnOnce(Vec<u8>),
{
    ...
    let verified = match verify_signed_message(&peer, &request, |err| {
        ...
        None => {
            let error_response = crate::protocol::machine::build_handshake(0, 0, "", "");
    ...
    match crate::protocol::machine::root_as_handshake(&effective_request) {
        Ok(_handshake_req) => {
            ...
            let handshake_response = crate::protocol::machine::build_handshake(
                rand::random::<u32>(),
                timestamp,
                "beemesh/1.0",
                &peer.to_string(),
...
pub fn handshake_response(
    ...
    // Parse the response
    match crate::protocol::machine::root_as_handshake(&effective_response) {
        Ok(_handshake_resp) => {

...
impl PodmanApiClient {
    ...
    pub async fn generate_kube(&self, pod_name: &str) -> Result<String, RuntimeError> {
        let api_version = self.ensure_api_version().await?;
        // The Podman API uses /generate/kube?names=POD_NAME, not /generate/POD_NAME/kube
        let uri = self.build_uri_with_query_for_version(
            &api_version,
            "/generate/kube",
            &format!("names={}", pod_name),
        );
...
mod tests {
    ...
    /// Tests that PODMAN_API_VERSION environment variable override works.
    ///
    /// # Safety
    /// Uses `#[serial]` to ensure single-threaded execution during env manipulation.
    /// Environment is restored after test completion.
    #[test]
    #[serial]
    fn test_env_override_sets_version() {
        let original = std::env::var(PODMAN_API_VERSION_ENV).ok();
        // SAFETY: Test runs with #[serial] ensuring single-threaded execution.
        // No other threads can observe the modified environment.
        unsafe {
        ...
        // SAFETY: Restore original environment state. Still single-threaded via #[serial].
        match original {
            Some(value) => unsafe {

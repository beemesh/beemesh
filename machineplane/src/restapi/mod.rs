use crate::protocol::libp2p_constants::{FREE_CAPACITY_PREFIX, FREE_CAPACITY_TIMEOUT_MS};
...
async fn get_nodes(
    ...
    let response_data = crate::protocol::machine::build_nodes_response(&peers);
...
async fn get_kem_public_key(State(_state): State<RestState>) -> String {
    ...
    match crate::crypto::ensure_kem_keypair_on_disk() {
...
pub async fn get_candidates(
    ...
    let response_data =
        crate::protocol::machine::build_candidates_response_with_keys(true, &candidates);
...
pub(crate) async fn collect_candidate_pubkeys(
    ...
    let capacity_fb = crate::protocol::machine::build_capacity_request(
        500u32,
        512u64 * 1024 * 1024,
...
pub async fn create_task(
    ...
    let (manifest_id, operation_id) = if let Some(id) = params.get("manifest_id") {
        ...
        if let Some(name) =
            crate::protocol::machine::extract_manifest_name(&payload_bytes_for_parsing)
        {
    ...
    let response_data = crate::protocol::machine::build_task_create_response(
        true,
        &task_id,
        &manifest_id,
        FREE_CAPACITY_TIMEOUT_MS,
...
async fn get_task_manifest_id(
    ...
    let task = match maybe {
        ...
        None => {
            let error_response =
                crate::protocol::machine::build_task_status_response("", "Error", &[], None);
...
pub async fn get_task_status(
    ...
    if let Some(r) = maybe {
        ...
        let response_data = crate::protocol::machine::build_task_status_response(
            &task_id,
            "Pending",
            &assigned,
...
pub async fn apply_direct(
    ...
    let target_peer_id: libp2p::PeerId = match peer_id.parse() {
        ...
        Err(e) => {
            ...
            let error_response = crate::protocol::machine::build_apply_response(
                false,
                "unknown",
    ...
    let result = match tokio::time::timeout(Duration::from_secs(30), reply_rx.recv()).await {
        Ok(Some(Ok(_msg))) => {
            ...
            crate::protocol::machine::build_apply_response(
                true,
                "forwarded",

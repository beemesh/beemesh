use crate::protocol::machine::{FbEnvelope, root_as_envelope};
...
impl EnvelopeHandler {
    ...
    pub async fn decrypt_request_envelope(&self, envelope_bytes: &[u8]) -> Result<Vec<u8>> {
        ...
        let decrypted_payload = crate::crypto::decrypt_payload_from_recipient_blob(
            &encrypted_payload,
    ...
    pub async fn encrypt_response_envelope_with_key(
        ...
        let encrypted_blob = crate::crypto::encrypt_payload_for_recipient(kem_pubkey, payload)?;
        ...
        let canonical_bytes = crate::protocol::machine::build_envelope_canonical(
            &encrypted_blob,
            payload_type,
            &nonce,
            ts,
        ...
        let (sig_b64, pub_b64) =
            crate::crypto::sign_envelope(signing_key, &public_key_bytes, &canonical_bytes)?;

        // Build the final signed envelope
        Ok(crate::protocol::machine::build_envelope_signed(
            &encrypted_blob,
            payload_type,
            &nonce,
            ts,
...
mod tests {
    ...
    use crate::crypto::{ensure_keypair_ephemeral, ensure_pqc_init};

    struct KeypairConfigGuard(crate::crypto::KeypairConfig);

    impl KeypairConfigGuard {
        fn kem_ephemeral() -> Self {
            let previous = crate::crypto::get_keypair_config();
            crate::crypto::set_keypair_config(crate::crypto::KeypairConfig {
                signing_mode: crate::crypto::KeypairMode::Ephemeral,
                kem_mode: crate::crypto::KeypairMode::Ephemeral,
    ...
    impl Drop for KeypairConfigGuard {
        fn drop(&mut self) {
            crate::crypto::set_keypair_config(self.0.clone());
        }
    }
    ...
    async fn test_envelope_roundtrip() {
        ...
        let (recipient_kem_pub, recipient_kem_priv) = crate::crypto::ensure_kem_keypair_on_disk()
            .expect("Failed to generate recipient KEM keypair");
        ...
        let encrypted_payload =
            crate::crypto::encrypt_payload_for_recipient(&recipient_kem_pub, payload)
        ...
        let (sig_b64, pub_b64) =
            crate::crypto::sign_envelope(&sender_priv, &sender_pub, &canonical)
                .expect("Failed to sign envelope");

        // Build the final signed envelope with encrypted payload
        let encrypted_envelope = crate::protocol::machine::build_envelope_signed(
            &encrypted_payload,
            payload_type,
            &nonce,
            timestamp,

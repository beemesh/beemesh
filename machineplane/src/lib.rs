...
//! - **messages**: Wire protocol message definitions
//! - **signatures**: Cryptographic signing/verification for messages
...
fn resolve_and_configure_podman_socket(
    ...
    // SAFETY: This unsafe block sets a process-wide environment variable.
    //
    // Why unsafe is required:
    //   `std::env::set_var` is unsafe in Rust 2024 edition because modifying
    //   environment variables is not thread-safe on some platforms.
    //
    // Why this is safe here:
    //   1. This runs during daemon startup, before any worker threads are spawned
    //   2. The tokio runtime has not yet started processing async tasks
    //   3. No concurrent reads of CONTAINER_HOST occur at this point
    //   4. Child processes (Podman) need this variable set before they're forked
    //
    // This follows the "set once at startup" pattern recommended for env vars.
    unsafe {
...
pub async fn start_machineplane(
    ...
    if let Err(e) = runtime::initialize_podman_manager().await {
        log::warn!(
...
mod tests {
    ...
    fn restore_env_var(key: &str, value: Option<String>) {
        // SAFETY: This unsafe block modifies environment variables in tests.
        //
        // Why this is safe:
        //   1. Tests using this function are marked with #[serial] to run sequentially
        //   2. No concurrent threads are accessing environment variables
        //   3. The serial_test crate ensures mutual exclusion between test functions
        //
        // This pattern is standard for testing code that depends on environment state.
        unsafe {
    ...
    fn podman_socket_absence_is_non_fatal() {
        ...
        // SAFETY: Test runs with #[serial], ensuring no concurrent env access.
        // See restore_env_var() for detailed safety rationale.
        unsafe {

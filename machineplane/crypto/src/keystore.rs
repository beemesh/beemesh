...
impl Keystore {
    ...
    pub fn open(path: &Path, ephemeral: bool) -> Result<Self> {
        let conn = if ephemeral || path.to_str() == Some(":memory:") {
            ...
            if let Ok(shared_name) = std::env::var("BEEMESH_KEYSTORE_SHARED_NAME") {
                ...
                info!("Keystore::open using shared temp file: {:?} thread_id={:?}", temp_path, std::thread::current().id());
                let flags = OpenFlags::SQLITE_OPEN_READ_WRITE
                    | OpenFlags::SQLITE_OPEN_CREATE
                    | OpenFlags::SQLITE_OPEN_FULL_MUTEX;
                let conn = Connection::open_with_flags(&temp_path, flags)?;
                info!("Keystore::open connection established for thread {:?}", std::thread::current().id());
                conn
            } else {
        ...
        let existing_count: i64 = conn.prepare("SELECT COUNT(*) FROM shares")?.query_row([], |row| row.get(0))?;
        info!("Keystore::open initialized with {} existing records thread_id={:?}", existing_count, std::thread::current().id());
    ...
    pub fn get(&self, cid: &str) -> Result<Option<Vec<u8>>> {
        let mut stmt = self.conn.prepare("SELECT blob FROM shares WHERE cid = ?1")?;

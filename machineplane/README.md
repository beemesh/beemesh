...
**Scheduling semantics:** Elections on the Machineplane are **hint-based**; **multiple concurrent lease hints MAY occur**. Exact leadership and write safety for stateful workloads are enforced by the **Workplane**.
...
* **Machine Daemon (Rust)**: single lightweight process per node (≈50–80 MB RAM). It **MAY** be packaged and run as an OCI **container**; when containerized it **MUST**:
...
### Feature: Ephemeral, Duplicate-Tolerant Scheduling
...
* [ ] **LeaseHints** with TTL for non-exclusive election hints (MUST)
* [ ] Deployment confirmation events (MUST)
* [ ] Backoff and retry after LeaseHint expiry (SHOULD/MAY as specified)
* [ ] MDHT stores only transient metadata (MUST)
* [ ] Workload DHT contains no machine data (MUST)
* [ ] Duplicate-tolerant semantics metering present (MUST)
* [ ] Daemon may run natively or as container with listed constraints (MUST)
...
* Multi-winner tasks (gang scheduling) and atomic co-scheduling (via Workplane hooks).
* Energy-aware scheduling (grid carbon intensity input).
* Formal modeling of duplicate-boundedness under churn/backoff strategies.

---

## 16. Change Log

* **v0.2**: Rust/libp2p profile; **CAS → LeaseHint (non-exclusive)**; duplicate-tolerant semantics; daemon vs container packaging; new metrics & scenarios.
* v0.1: Initial draft with exclusive CAS language.

---

### Appendix A (Informative): Rust Implementation Profile

* **Crates:** `libp2p` (gossipsub, kad, noise/tls, identify, ping), `tokio`, `tracing`, `flatbuffers`, `serde`, `ulid`, `ed25519-dalek`.
* **MDHT:** `libp2p-kad` provider/record store for presence; key/value `lease/<task_id>` with TTL.
* **Pub/Sub:** `libp2p-gossipsub` with validation; `MessageId` de-dup keyed by `(topic, task_id)`.
* **Backoff:** jittered exponential (cap ~1.5–5 s) keyed by `task_id`.
* **Security:** Noise XX/IK; Ed25519 message signatures; replay windows per spec.
* **Runtime adapter:** async Podman invocations; log streaming over framed channels.
